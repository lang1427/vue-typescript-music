...
const state: IState = {
  ...
  loginAccount: window.sessionStorage.getItem("loginAccount")
  ...
  currentPlayIndex: (<any>window.localStorage).getItem("playIndex")
    ? parseInt((<any>window.localStorage).getItem("playIndex"))
    : -1,
  // currentPlayIndex: parseInt((<any>window.localStorage).getItem('playIndex')) || -1, // 当前播放容器的索引值
  playMode:
    parseInt((<any>window.localStorage).getItem("mode")) || EPlayMode.listLoop,
