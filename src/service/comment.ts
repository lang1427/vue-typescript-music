...
export class CommentClass {
  ...
  public commentLikedCount: number;

  public parentCommentId: number;

  public replyUserId: number;
  public replyUserName: string;
  public replyUserAvatar: string;
  ...
  constructor(commentObj: IComment) {
    ...
    this.replyUserId =
      commentObj.beReplied &&
      commentObj.beReplied[0] &&
      commentObj.beReplied[0].user.userId;
    this.replyUserName =
      commentObj.beReplied &&
      commentObj.beReplied[0] &&
      commentObj.beReplied[0].user.nickname;
    this.replyUserAvatar =
      commentObj.beReplied &&
      commentObj.beReplied[0] &&
      commentObj.beReplied[0].user.avatarUrl;
    this.replyContent =
      commentObj.beReplied &&
      commentObj.beReplied[0] &&
      commentObj.beReplied[0].content;
  }
}

// 歌单评论 (歌单id，偏移数量：page*limit，limit：数量)
export function songSheetComment(
  id: number,
  offset?: number,
  limit: number = 20
) {
